...
char Yorp::color() const
{
    ...
    // Delete the following line and replace it with the correct code.
    return m_color;  // This implementation compiles, but is incorrect.
}
...
string Player::move(int dir)
{
    ...
    if(!attemptMove(*m_arena, dir, m_row, m_col))
        return "Player couldn't move; player stands.";
    else if((*m_arena).numberOfYorpsAt(m_row, m_col)>0)
    {
        setDead();
        return "Player walked into a yorp and died.";
    }
    
    //use switch statement to cout different sentences
    switch (dir) {
        case 0:
...
Arena::~Arena()
{
    // TODO:  Release the player and all remaining dynamically allocated yorps.
    delete m_player;
    for(int t=0;t<m_nYorps;t++)
        delete m_yorps[t];
}
...
int Arena::yorpCount() const
{
    ...
    // Delete the following line and replace it with the correct code.
    return m_nYorps;  // This implementation compiles, but is incorrect.
}
...
int Arena::numberOfYorpsAt(int r, int c) const
{
    ...
    for(int t=0;t<m_nYorps;t++)
    {
        if((m_yorps[t]->row()==r)&&(m_yorps[t]->col()==c))
            number++;
    }
        
    return number;  // This implementation compiles, but is incorrect.
}
...
void Arena::display(string msg) const
{
    ...
    for(int t=0;t<m_nYorps;t++)
    {
        ...
        displayGrid[(*m_yorps[t]).row()-1][(*m_yorps[t]).col()-1]=(m_yorps[t]->color());
    }
...
string Arena::moveYorps(char color, int dir)
{
    ...
    if(willFollow)
    {
        ...
        for(int t=0;t<m_nYorps;t++)
        {
            if(m_yorps[t]->color()==color)
            {
                m_yorps[t]->forceMove(dir);
                if((m_player->col()==m_yorps[t]->col())&&(m_player->row()==m_yorps[t]->row()))
                    m_player->setDead();
            }
            else
            {
...
bool attemptMove(const Arena& a, int dir, int& r, int& c)
{
    ...
    // Delete the following line and replace it with the correct code.
    //use switch statement to detect different dir it gives
    switch (dir) {
        case 0:
        {
            if ((r-1)<1)
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    ...
    //there are five options, stay, move north, move East, move west and move south
    if(r-1>0)
    {
        if(a.numberOfYorpsAt(r-1, c)>0)
        {
            N=3642;
            stay+=a.numberOfYorpsAt(r-1, c);
        }
        else if(a.hasWallAt(r-1, c))
            N=1000;
        else
        {
            ...
            {
                if(a.numberOfYorpsAt(r-1, c)>0)
                    N+=a.numberOfYorpsAt(r-1, c);
            }
            if(c+1<=a.cols())
            {
                if(a.numberOfYorpsAt(r, c+1)>0)
                    N+=a.numberOfYorpsAt(r, c+1);
            }
            if(c-1>0)
            {
                if(a.numberOfYorpsAt(r, c-1)>0)
    ...
    if(r+1<=a.rows())
    {
        ...
        else
        {
            ...
            {
                if(a.numberOfYorpsAt(r+1, c)>0)
                    S+=a.numberOfYorpsAt(r+1, c);
            }
            if(c+1<=a.rows())
            {
                if(a.numberOfYorpsAt(r, c+1)>0)
                    S+=a.numberOfYorpsAt(r, c+1);
            }
            if(c-1>0)
            {
                if(a.numberOfYorpsAt(r, c-1)>0)
    ...
    if(N==3642&&E==3642&&W==3642&&S==3642)
        return false;
    if(stay<=N&&stay<=E&&stay<=W&&stay<=S)
        return false;
    else if(N==1000&&E==1000&&W==1000&&S==1000)
    {
        return false;
    }
    else if(N==1000&&E!=0&&W!=0&&S!=0)
    {
        if(stay<=N&&stay<=E&&stay<=W&&stay<=S)
        return false;
    }
    else if(S==1000&&E!=0&&W!=0&&N!=0)
    {
        if(stay<=N&&stay<=E&&stay<=W&&stay<=S)
            return false;
    }
    else if(W==1000&&E!=0&&S!=0&&N!=0)
    {
        if(stay<=N&&stay<=E&&stay<=W&&stay<=S)
            return false;
    }
    else if(E==1000&&S!=0&&W!=0&&N!=0)
    {
        if(stay<=N&&stay<=E&&stay<=W&&stay<=S)
            return false;
    }
